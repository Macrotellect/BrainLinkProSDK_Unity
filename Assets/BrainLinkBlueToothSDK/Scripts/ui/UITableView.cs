...
public class UITableView : MonoBehaviour {
	...
	void onScroll(Vector2 value) {
		Debug.Log("_totalCellCount: " + _totalCellCount + "_displayCellCount: " + _displayCellCount);
        if (_totalCellCount <= _displayCellCount)
        {
            return;
        }
	...
	void reuse(int minIndex, int maxIndex) {
		...
		Action<int, int> doReuse = (int reuseIndex, int targetIndex) => {
			Debug.Log("reuseIndex: " + reuseIndex + "target: " + targetIndex+ "_cells.count: " + _cells.Count);
			if (!_cells.ContainsKey(reuseIndex)) {
				return;
			}
	...
	public void setIsHaveButton(bool isHave)
	{
		_isHaveBtn = isHave;
		Debug.Log("isHave---" + _isHaveBtn);
	}
	...
	public void reload(int size) {
		...
		if (_isVertical) {
			...
			int addAlpDis = 0;
			if (_totalCellCount >= _displayCellCount-1)
			{
				addAlpDis = 40;
			}
			_contentRect.sizeDelta = new Vector2 (_contentRect.sizeDelta.x, _totalCellCount * _cellSize.y+ addAlpDis);
		...
		int count = _totalCellCount > _displayCellCount ? _displayCellCount : _totalCellCount - 1;
		...
		for (int i = 0; i < count + 1; ++i) {
			...
			if (_isHaveBtn) {
				cell.transform.Find("Button").gameObject.GetComponent<Button>().onClick.AddListener(() =>
				{
					onClick(cell);
				});
			}
